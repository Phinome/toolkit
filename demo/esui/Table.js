...
esui.Table.Editor.prototype   = {
    ...
    start: function ( table, options ) {
        ...
        var maxWidth = T.dom.getPosition( table.main ).left + table.main.offsetWidth;

        if (left + this.layer.main.offsetWidth > maxWidth) {
            left = maxWidth - this.layer.main.offsetWidth;
        }
...
// 初始化内建表格编辑部件 - float类型
esui.Table.EditorManager.add('float', new esui.Table.Editor( {
    /**
     * 编辑器类型
     *
     * @public
     ...
     */
    getValue: function () {
        return parseFloat(this.inputCtrl.getValue());
    },
    ...
    getOkHandler: function () {
        ...
        return function () {
            var value = me.inputCtrl.getValue();
            if (!/(^[0-9]\d*$)|(^[0-9]\d*.?\d{1,2}$)/.test(value)) {
                me.setError('请输入正确的数字，最多两位小数。');
                return;
            }
            me.doOk();
        };
