...
esui.Pager = function(options) {
    ...
    this.pageSize = parseInt(this.pageSize, 10) || 10;
};
...
esui.Pager.prototype = {
    ...
    render: function () {
        ...
        me.pageCount =  Math.ceil(this.total / this.pageSize) || 0;
    ...
    _renderPages: function () {
        var me        = this,
            ...
            last      = pageCount + startNumber - 1,
            page      = me.page - startNumber, // 恶心 //fixed by fedeoo 应该是减去startNumber 而非加上
        ...
        if ( page < me.showCount - 1 ) {
            begin = 0;
        } else if (page > pageCount - me.showCount) {
            begin = pageCount - me.showCount;
        } else {
        ...
        // 绘制中间的序号
        for ( i = 0; i < me.showCount && begin + i < pageCount; i++ ) {
            if ( begin + i != page ) {
                html.push(
                    me._getItemHtml(
                        1 + begin + i,
        ...
        // 绘制后缀
        if ( begin < pageCount - me.showCount ) {
            html.push(
                omitWord,
                me._getItemHtml(
                    pageCount,
                    itemClass,
                    me.__getStrCall( '_setPage', last - startNumber) // fixed by fedeoo
                ) );
        }


        // 绘制后一页的link
        if ( page + startNumber < last ) { // fixed by fedeoo
            html.push(
                me._getItemHtml(
                    me.nextText,
                    nextClass,
